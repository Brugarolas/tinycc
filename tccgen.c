...
/* rotate n first stack elements to the top
   I1 ... In -> In I1 ... I(n-1)  [top is right]
 */
ST_FUNC void vrott(int n) { vrote(vtop, n); }
...
/* find a free temporary local variable (return the offset on stack) match the
...
static void clear_temp_local_var_list() { nb_temp_local_vars = 0; }
  ...
  if (c1 && c2) {
    ...
    case '/':
      if (f2 == 0.0) {
        ...
        if (!CONST_WANTED)
          goto general_case;
        /* the run-time result of 0.0/0.0 on x87, also of other
           compilers when used to compile the f1 /= f2 below, would be
           -nan */
        x1.f = f1, x2.f = f2;
        if (f1 == 0.0)
          y.u = 0x7fc00000; /* nan */
      ...
      if (pbt1 != VT_VOID && pbt2 != VT_VOID &&
        ...
        if (op != '?' && !TOK_ISCOND(op))
          ...
          type_incompatibility_warning(
              type1, type2,
              op == '?' ? "pointer type mismatch in conditional "
                          "expression ('%s' and '%s')"
                        : "pointer type mismatch in comparison('%s' "
                          "and '%s')");
      }
      if (op == '?') {
...
static int post_type(CType *type, AttributeDef *ad, int storage, int td) {
  ...
  if (tok == '(') {
    ...
    if (l) {
      for (;;) {
        ...
        /* these symbols may be evaluated for VLArrays (see below, under
           nocode_wanted) which is why we push them here as normal
           symbols temporarily.  Example: int func(int a, int b[++a]);
         */
    ...
    if (td & TYPE_PARAM)
      while (1) {
        /* XXX The optional type-quals and static should only be
           accepted in parameter decls.  The '*' as well, and then even
           only in prototypes (not function defs).  */
        switch (tok) {
        case TOK_RESTRICT1:
        case TOK_RESTRICT2:
        case TOK_RESTRICT3:
...
tok_next:
  ...
  case TOK_builtin_constant_p:
    ...
    n = constant_p && (vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST &&
  ...
  case TOK_builtin_return_address: {
    ...
    if (level < 0) {
      tcc_error("%s only takes positive integers",
                tok1 == TOK_builtin_return_address ? "__builtin_return_address"
...
static void expr_cond(void) {
  ...
  if (tok == '?') {
    ...
    if (c < 0 && is_cond_bool(vtop) && is_cond_bool(&sv)) {
      /* optimize "if (f ? a > b : c || d) ..." for example, where
         normally "a < b" and "c || d" would be forced to "(int)0/1"
         first, whereas this code jumps directly to the if's then/else
         branches. */
      t1 = gvtst(0, 0);
      t2 = gjmp(0);
      gsym(u);
    ...
    /* keep structs lvalue by transforming `(expr ? a : b)` to `*(expr ? &a
       : &b)` so that `(expr ? a : b).mem` does not error  with "lvalue
       expected" */
  ...
  skip_switch:
    ...
    if (gnu_ext && tok == TOK_DOTS) {
      ...
      if ((!(cur_switch->sv.type.t & VT_UNSIGNED) && cr->v2 < cr->v1) ||
          (cur_switch->sv.type.t & VT_UNSIGNED &&
           (uint64_t)cr->v2 < (uint64_t)cr->v1))
    ...
    if ((vtop->r & (VT_SYM | VT_CONST)) == (VT_SYM | VT_CONST) &&
      ...
      if (type->t & VT_BITFIELD) {
        ...
        case VT_LDOUBLE:
          ...
          /* Host and target platform may be different but both have
             x87. On windows, tcc does not use VT_LDOUBLE, except when
             it is a cross compiler.  In this case a mingw gcc as host
             compiler comes here with 10-byte long doubles, while msvc
             or tcc won't. tcc itself can still translate by asm. In
             any case we avoid possibly random bytes 11 and 12.
          */
...
#endif
            /* For other platforms it should work natively, but may
               not work for cross compilers */
...
#if PTR_SIZE == 8
        /* intptr_t may need a reloc too, see
         * tcctest.c:relocation_test() */
        case VT_LLONG:
        case VT_PTR:
          if (vtop->r & VT_SYM)
        ...
        found:
          ...
          if (((type.t & VT_EXTERN) && (!has_init || l != VT_CONST)) ||
            ...
            if (ad.alias_target) {
              ...
              put_extern_sym2(sym, esym->st_shndx, esym->st_value,
                              esym->st_size, 1);
            }
          } else {
